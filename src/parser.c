...
// parser functions
// `static Code* parse_code(Lexer* lexer, Scope* now_scope)`
static offset(Code*) parse_code(offset(Lexer*) lexer, offset(Scope*) now_scope);
...
// `static Function* parse_function(Lexer* lexer, Scope* now_scope)`
static offset(Function*) parse_function(offset(Lexer*) lexer, offset(Scope*) now_scope);
// `static Method* parse_method(Lexer* lexer, Scope* now_scope, Name* class_name)`
static offset(Method*) parse_method(offset(Lexer*) lexer, offset(Scope*) now_scope, offset(Name*) class_name);
// `static Class* parse_class(Lexer* lexer, Scope* now_scope)`
static offset(Class*) parse_class(offset(Lexer*) lexer, offset(Scope*) now_scope);
// `static Variable* parse_variable(Lexer* lexer, Scope* now_scope)`
static offset(Variable*) parse_variable(offset(Lexer*) lexer, offset(Scope*) now_scope);
// `static Statement* parse_statement(Lexer* lexer, Scope* now_scope)`
static offset(Statement*) parse_statement(offset(Lexer*) lexer, offset(Scope*) now_scope);
// `static If* parse_if(Lexer* lexer, Scope* now_scope)`
static offset(If*) parse_if(offset(Lexer*) lexer, offset(Scope*) now_scope);
// `static For* parse_for(Lexer* lexer, Scope* now_scope)`
static offset(For*) parse_for(offset(Lexer*) lexer, offset(Scope*) now_scope);
// `static While* parse_while(Lexer* lexer, Scope* now_scope)`
static offset(While*) parse_while(offset(Lexer*) lexer, offset(Scope*) now_scope);
// `static Expression* parse_expression(Lexer* lexer, Scope* now_scope)`
static offset(Expression*) parse_expression(offset(Lexer*) lexer, offset(Scope*) now_scope);
// `static Primary* parse_primary(Lexer* lexer, Scope* now_scope)`
static offset(Primary*) parse_primary(offset(Lexer*) lexer, offset(Scope*) now_scope);
// `static VariableAccess* parse_variable_access(Lexer* lexer, Scope* now_scope)`
static offset(VariableAccess*) parse_variable_access(offset(Lexer*) lexer, offset(Scope*) now_scope);
...
offset(Code*) parse_code(offset(Lexer*) lexer, offset(Scope*) now_scope) {
    ...
    while (token != 0 && token_ptr->type != EOF_TOKEN) {
        if (token_ptr->type == KEYWORD && string_equal(token_ptr->lexeme, IMPORT_KEYWORD)) {
            ...
            offset(Function*) function = parse_function(lexer, global_scope);
            if (function == 0)
            ...
            offset(Class*) class_ = parse_class(lexer, global_scope);
            if (class_ == 0)
...
offset(Import*) parse_import(offset(Lexer*) lexer, offset(Scope*) now_scope) {
    ...
    if (strcmp(string_to_cstr(source), "std") == 0) {
        name = create_name(import_name, NAME_FUNCTION, name_void, now_scope);
    } else
        name = create_name(import_name, NAME_TYPE, 0, now_scope);
    return create_import(name, source);
}
offset(Function*) parse_function(offset(Lexer*) lexer, offset(Scope*) now_scope) {
    ...
    while (token_ptr->type != SYMBOL || !string_equal(token_ptr->lexeme, R_PAREN_SYMBOL)) {
        offset(Variable*) parameter = parse_variable(lexer, function_scope);
        if (parameter == 0)
    ...
    while (token_ptr->type != SYMBOL || !string_equal(token_ptr->lexeme, R_BRACE_SYMBOL)) {
        offset(Statement*) statement = parse_statement(lexer, function_scope);
        if (statement == 0)
...
offset(Method*) parse_method(offset(Lexer*) lexer, offset(Scope*) now_scope, offset(Name*) class_name) {
    ...
    while (token_ptr->type != SYMBOL || !string_equal(token_ptr->lexeme, R_PAREN_SYMBOL)) {
        ...
        offset(Variable*) parameter = parse_variable(lexer, method_scope);
        if (parameter == 0)
    ...
    next_token(lexer);
    while (token_ptr->type != SYMBOL || !string_equal(token_ptr->lexeme, R_BRACE_SYMBOL)) {
        offset(Statement*) statement = parse_statement(lexer, method_scope);
        if (statement == 0)
...
offset(Class*) parse_class(offset(Lexer*) lexer, offset(Scope*) now_scope) {
    ...
    while (token_ptr->type != SYMBOL || !string_equal(token_ptr->lexeme, R_BRACE_SYMBOL)) {
        if (token_ptr->type == KEYWORD && string_equal(token_ptr->lexeme, METHOD_KEYWORD)) {
            offset(Method*) method = parse_method(lexer, class_scope, name);
            if (method == 0)
            ...
            next_token(lexer);
            offset(Variable*) variable = parse_variable(lexer, class_scope);
            if (variable == 0)
...
offset(Variable*) parse_variable(offset(Lexer*) lexer, offset(Scope*) now_scope) {
    peek_current_token();
    if (token_ptr->type != IDENTIFIER && !(token_ptr->type == KEYWORD && is_builtin_type(token_ptr->lexeme)))
        return parser_error("Expected variable type", token);
    offset(Name*) type = search(now_scope, token_ptr->lexeme);
    if (type == 0)
        return parser_error("Unknown variable type", token);
    next_token(lexer);
    if (token_ptr->type != IDENTIFIER)
    ...
    if (token_ptr->type == SYMBOL && string_equal(token_ptr->lexeme, ASSIGN_SYMBOL)) {
        ...
        next_token(lexer);
        value = parse_expression(lexer, now_scope);
        if (value == 0)
            parser_error("Failed to parse variable initializer", token);
    }
    return create_variable(type, name, value);
}
offset(Statement*) parse_statement(offset(Lexer*) lexer, offset(Scope*) now_scope) {
    peek_current_token();
    offset(Statement*) statement = 0;
    if (token_ptr->type == KEYWORD) {
        if (string_equal(token_ptr->lexeme, IF_KEYWORD))
            return create_statement(IF_STATEMENT, parse_if(lexer, now_scope));
        else if (string_equal(token_ptr->lexeme, FOR_KEYWORD))
            return create_statement(FOR_STATEMENT, parse_for(lexer, now_scope));
        else if (string_equal(token_ptr->lexeme, WHILE_KEYWORD))
            return create_statement(WHILE_STATEMENT, parse_while(lexer, now_scope));
        else if (string_equal(token_ptr->lexeme, VAR_KEYWORD)) {
            next_token(lexer);
            statement = create_statement(VARIABLE_STATEMENT, parse_variable(lexer, now_scope));
        } else if (string_equal(token_ptr->lexeme, RETURN_KEYWORD)) {
            next_token(lexer);
            statement = create_statement(RETURN_STATEMENT, parse_expression(lexer, now_scope));
            ...
            statement = create_statement(EXPRESSION_STATEMENT, parse_expression(lexer, now_scope));
    } else
        statement = create_statement(EXPRESSION_STATEMENT, parse_expression(lexer, now_scope));
    if (statement == 0)
...
offset(If*) parse_if(offset(Lexer*) lexer, offset(Scope*) now_scope) {
    next_token(lexer);
    if (token_ptr->type != SYMBOL || !string_equal(token_ptr->lexeme, L_PAREN_SYMBOL))
        return parser_error("Expected '(' after 'if'", token);
    next_token(lexer);
    offset(Expression*) condition = parse_expression(lexer, now_scope);
    if (condition == 0)
    ...
    next_token(lexer);
    while (token_ptr->type != SYMBOL || !string_equal(token_ptr->lexeme, R_BRACE_SYMBOL)) {
        offset(Statement*) statement = parse_statement(lexer, now_scope);
        if (statement == 0)
    ...
    while (token_ptr->type == KEYWORD && string_equal(token_ptr->lexeme, ELIF_KEYWORD)) {
        ...
        next_token(lexer);
        offset(Expression*) elif_condition = parse_expression(lexer, now_scope);
        if (elif_condition == 0)
...
offset(For*) parse_for(offset(Lexer*) lexer, offset(Scope*) now_scope) {
    next_token(lexer);
    if (token_ptr->type != SYMBOL || !string_equal(token_ptr->lexeme, L_PAREN_SYMBOL))
    ...
    if (token_ptr->type != SYMBOL || !string_equal(token_ptr->lexeme, SEMICOLON_SYMBOL)) {
        initializer = parse_variable(lexer, now_scope);
        if (initializer == 0)
    ...
    if (token_ptr->type != SYMBOL || !string_equal(token_ptr->lexeme, SEMICOLON_SYMBOL)) {
        condition = parse_expression(lexer, now_scope);
        if (condition == 0)
    ...
    if (token_ptr->type != SYMBOL || !string_equal(token_ptr->lexeme, R_PAREN_SYMBOL)) {
        increment = parse_expression(lexer, now_scope);
        if (increment == 0)
...
offset(While*) parse_while(offset(Lexer*) lexer, offset(Scope*) now_scope) {
    next_token(lexer);
    if (token_ptr->type != SYMBOL || !string_equal(token_ptr->lexeme, L_PAREN_SYMBOL))
...
static offset(Expression*) parse_expr_prec(offset(Lexer*) lexer, offset(Expression*) left, int min_prec, offset(Scope*) now_scope) {
    while ((void)(peek_token(lexer)), token_ptr->type == SYMBOL) {
        ...
        next_token(lexer);
        offset(Primary*) right_primary = parse_primary(lexer, now_scope);
        if (right_primary == 0)
        ...
        while ((void)(peek_token(lexer)), token_ptr->type == SYMBOL) {
            ...
            if (next_op == OP_NONE || next_prec <= op_prec)
                break;
            right = parse_expr_prec(lexer, right, next_prec, now_scope);
            if (right == 0)
                return 0;
        }
...
offset(Expression*) parse_expression(offset(Lexer*) lexer, offset(Scope*) now_scope) {
    offset(Primary*) left_primary = parse_primary(lexer, now_scope);
    if (left_primary == 0)
        return parser_error("Failed to parse expression primary", token);
    return parse_expr_prec(lexer, create_expression(OP_NONE, left_primary, 0), 0, now_scope);
}
offset(Primary*) parse_primary(offset(Lexer*) lexer, offset(Scope*) now_scope) {
    peek_current_token();
    PrimaryType type;
    ...
    if (token_ptr->type == INTEGER) {
        ...
        next_token(lexer);
        value = parse_primary(lexer, now_scope);
        if (value == 0)
        ...
        if (value == 0)
            return parser_error("Failed to parse operand of unary '-'", token);
    } else if (token_ptr->type == IDENTIFIER || (token_ptr->type == KEYWORD && string_equal(token_ptr->lexeme, SELF_KEYWORD) && in_method)) {
        type = PRIM_VARIABLE_ACCESS;
        value = parse_variable_access(lexer, now_scope);
        if (value == 0)
...
offset(VariableAccess*) parse_variable_access(offset(Lexer*) lexer, offset(Scope*) now_scope) {
    ...
    while (token_ptr->type == SYMBOL) {
        ...
        if (string_equal(token_ptr->lexeme, L_PAREN_SYMBOL)) {
            ...
            while (token_ptr->type != SYMBOL || !string_equal(token_ptr->lexeme, R_PAREN_SYMBOL)) {
                offset(Expression*) arg = parse_expression(lexer, now_scope);
                if (arg == 0)
            ...
            next_token(lexer);  // consume '['
            next_token(lexer);
            offset(Expression*) index = parse_expression(lexer, now_scope);
            if (index == 0)
