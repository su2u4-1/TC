...
static Var* create_var(Symbol* original_name, Symbol* type, VarType kind, TACStatus* status) {
    ...
    if (id == (size_t)-1)
        sprintf(var->name, "$%u-error", kind);
    else
        sprintf(var->name, "$%c%zu", (char)kind, id);
    var->type = type;
...
static Arg* create_arg(ArgType kind, void* value) {
    ...
    switch (kind) {
        ...
        case ARG_NONE:
        default:
            fprintf(stderr, "[Warning] Unsupported argument type for create_arg: %u\n", kind);
            break;
    }
    arg->is_get = 0;
    return arg;
...
void tac_import(Import* import, TAC* tac, TACStatus* status) {
    if (import->name->kind == SYMBOL_VARIABLE)
        ...
        fprintf(stderr, "[Warning] Unsupported symbol kind for import: %u\n", import->name->kind);
}
void tac_function(Function* function, TACStatus* status) {
    ...
    while ((member = (ClassMember*)list_pop(members)) != NULL) {
        switch (member->type) {
            ...
            case CLASS_VARIABLE:
                ...
                break;
            default:
                fprintf(stderr, "[Warning] Unsupported class member type for tac_class: %u\n", member->type);
                break;
        }
    }
...
void tac_statement(Statement* statement, TACStatus* status) {
    switch (statement->type) {
        ...
        case CONTINUE_STATEMENT:
            ...
            break;
        default:
            fprintf(stderr, "[Warning] Unsupported statement type for tac_statement: %u\n", statement->type);
            break;
    }
}
void tac_if(If* if_, TACStatus* status) {
...
Arg* tac_primary(Primary* primary, TACStatus* status) {
    switch (primary->type) {
        ...
        default:
            fprintf(stderr, "[Warning] Unsupported primary type for tac_primary: %u\n", primary->type);
            return NULL;
    }
}
Arg* tac_variable_access(VariableAccess* variable_access, TACStatus* status) {
